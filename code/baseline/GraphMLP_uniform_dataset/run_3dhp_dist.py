    ...
    for i, data in enumerate(tqdm(dataLoader, 0)):
        ...
        if split == 'train':
            ...
            if opt.MAE:
                ...
                action_error_sum = test_calculation(pred_out, out_target, action, action_error_sum, opt.dataset, subject=None)
                joint_error_test = mpjpe_cal(pred_out, out_target).item()
                joint_error_test_p2 = p_mpjpe_torch(pred_out.squeeze(1), out_target.squeeze(1)).mean()
                out = pred_out
                # if opt.refine:
                #     post_out[:, :, 14, :] = 0
...
if __name__ == '__main__':
    ...
    if opt.train:
        ...
        train_dataloader = torch.utils.data.DataLoader(train_data, batch_size=opt.batchSize,
                                                       shuffle=False, sampler=train_sampler, # TODO
    ...
    model['trans'] = DistributedDataParallel(model['trans'], device_ids=[local_rank], find_unused_parameters=True)  # TODO
    model['refine'] = DistributedDataParallel(model['refine'], device_ids=[local_rank], find_unused_parameters=True)  # TODO
    model['MAE'] = DistributedDataParallel(model['MAE'], device_ids=[local_rank], find_unused_parameters=True)  # TODO
