...
if __name__ == '__main__':
    ...
    if opt.train:
        ...
        train_dataloader = torch.utils.data.DataLoader(train_data, batch_size=opt.batch_size,
                                                       shuffle=False, sampler=train_sampler, num_workers=int(opt.workers), pin_memory=True) #TODO

    test_data = Fusion(opt=opt, train=False, dataset=dataset, root_path =root_path)
    test_sampler = DistributedSampler(test_data)#TODO
    test_dataloader = torch.utils.data.DataLoader(test_data, batch_size=opt.batch_size,
                                                  shuffle=False, sampler=test_sampler, num_workers=int(opt.workers), pin_memory=True) #TODO

    model = Model(opt).cuda()
    # model = SyncBatchNorn.convert_sync_batchnorm(model) #TODO
