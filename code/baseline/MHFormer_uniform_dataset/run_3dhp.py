...
def step(split, opt, actions, dataLoader, model, optimizer=None, epoch=None):
    ...
    for i, data in enumerate(tqdm(dataLoader, 0)):
        ...
        if opt.MAE:
            ...
            if opt.test_augmentation and split =='test':
                ...
                input_2D = input_2D.view(N, -1, opt.n_joints, opt.in_channels, 1).permute(0, 3, 1, 2, 4).type(
                    torch.cuda.FloatTensor)
                # torch.Size([1024, 2, 27, 17, 1])
                # print(input_2D.shape)
                input_2D_vp3d = input_2D.squeeze(4).permute(0, 2, 3, 1)
                # print(input_2D.shape)
                output_3D, output_3D_VTE = model_trans(input_2D_vp3d)
                # print(output_3D.shape)
                output_3D = output_3D.unsqueeze(4).permute(0, 3, 1, 2, 4)
            ...
            if opt.refine:
                ...
                if split == 'train':
                    loss = mpjpe_cal(output_3D_single[:, 13, :, :].unsqueeze(1), out_target_single)
                    # loss = mpjpe_cal(output_3D_single, out_target_single)
                elif split == 'test':
                    # print("pred_out", pred_out.shape)
                    # print("out_target", out_target.shape)
                    # pred_out_single = pred_out[:, 13, :, :].unsqueeze(1)
                    # print("pred_out_single", pred_out_single.shape)
                    
                    # output_3D_single_single = output_3D_single[:, 13, :, :].unsqueeze(1)
                    # print("output_3D_single", output_3D_single.shape)
                    # print("out_target_single", out_target_single.shape)
                    # print("output_3D_single_single", output_3D_single_single.shape)
                    loss = mpjpe_cal(pred_out[:, 13, :, :].unsqueeze(1), out_target) + mpjpe_cal(output_3D_single[:, 13, :, :].unsqueeze(1), out_target_single)
                    # loss = mpjpe_cal(pred_out_single, out_target) + mpjpe_cal(output_3D_single_single, out_target_single)
        ...
        if split == 'train':
            ...
            if not opt.MAE:
                ...
                if opt.refine:
                    ...
                    joint_error = mpjpe_cal(pred_out[:, 13, :, :].unsqueeze(1), out_target_single).item()
            ...
            if opt.MAE:
                ...
                pred_out[:, :, 14, :] = 0
                # action = ['TestAction' for a in range(0, 1024)]
                action = seq[1]
                # print(type(action))
                # print(len(action))
                # print(action)
                action_error_sum = test_calculation(pred_out[:, 13, :, :].unsqueeze(1), out_target, action, action_error_sum, opt.dataset, subject=None)
                
                joint_error_test = mpjpe_cal(pred_out[:, 13, :, :].unsqueeze(1), out_target).item()
                # print("pred_out", pred_out.shape)
                # print("out_target", out_target.shape)
                joint_error_test_p2 = p_mpjpe_torch(pred_out[:, 13, :, :].squeeze(1), out_target.squeeze(1)).mean()
            ...
            error_sum_test_p2.update(joint_error_test_p2 * N, N)
    ...
    if split == 'train':
        ...
        return error_sum_test.avg, error_sum_test_p2.avg
...
def input_augmentation(input_2D, model_trans, joints_left, joints_right):
    ...
    input_2D_flip_vp3d = input_2D_flip.squeeze(4).permute(0, 2, 3, 1)
    input_2D_non_flip_vp3d = input_2D_non_flip.squeeze(4).permute(0, 2, 3, 1)
    
    output_3D_flip, output_3D_flip_VTE = model_trans(input_2D_flip_vp3d)
    ...
    output_3D_non_flip, output_3D_non_flip_VTE = model_trans(input_2D_non_flip_vp3d)
...
if __name__ == '__main__':
    ...
    # filter_widths = [int(x) for x in opt.architecture.split(',')]
        ...
        if opt.test == 1:
            ...
            if opt.train == 0:
                ...
                if opt.MAE:
                    ...
                    print('e: %d, lr: %.7f, loss: %.4f, MPJPE: %.2f, p1: %.2f, p2: %.2f' % (epoch, lr, loss, mpjpe, p1, p2))
