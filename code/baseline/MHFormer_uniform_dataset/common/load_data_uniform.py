...
class Fusion(data.Dataset):
    ...
    def prepare_data(self, path, train=True):
        ...
        if train == True:
            data_3d = np.load(path+"data_train_uniform.npz", allow_pickle=True)['train_3d'].item()
            data_2d = np.load(path + "data_train_uniform.npz", allow_pickle=True)['train_2d'].item()
            ...
            data_3d = np.load(path + "data_test_uniform.npz", allow_pickle=True)['test_3d'].item()
            data_2d = np.load(path + "data_test_uniform.npz", allow_pickle=True)['test_2d'].item()
