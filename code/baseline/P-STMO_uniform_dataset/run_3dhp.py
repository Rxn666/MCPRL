...
def step(split, opt, actions, dataLoader, model, optimizer=None, epoch=None):
    ...
    if split == 'train':
        ...
        if opt.MAE:
            ...
            return error_sum_test.avg*1000, 0
        if opt.refine:
    ...
    for epoch in range(1, opt.nepoch):
        ...
        if opt.test == 1:
            ...
            if opt.train == 0:
                ...
                # print(type(p1))
                if opt.MAE:
                    logging.info('epoch: %d, lr: %.7f, loss: %.4f, p1: %.2f' % (epoch, lr, loss, p1))
                    print('e: %d, lr: %.7f, loss: %.4f, p1: %.2f' % (epoch, lr, loss, p1))
                else:
                    logging.info('epoch: %d, lr: %.7f, loss: %.4f, MPJPE: %.2f, p1: %.2f, p2: %.2f' % (
                    epoch, lr, loss, mpjpe, p1, p2))
                    print('e: %d, lr: %.7f, loss: %.4f, MPJPE: %.2f, p1: %.2f, p2: %.2f' % (
                    epoch, lr, loss, mpjpe, p1, p2))
