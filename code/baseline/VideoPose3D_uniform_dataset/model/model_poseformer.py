...
class Model(nn.Module):
    def __init__(self, opt, num_frame=9, num_joints=17, in_chans=2,
        ...
        # self.Spatial_pos_embed_ = nn.Parameter(torch.zeros(1, num_joints, embed_dim_ratio))
    ...
    def Spatial_forward_features(self, x):
        ...
        # index = torch.arange(0, f, (f-1)//(self.num_frame_kept-1))
    ...
    def forward_features(self, x, Spatial_feature):
        ...
        # x = x.permute(0, 3, 1, 2).contiguous().view(b*num_coeff_kept, p, -1)

        # x = self.Freq_embedding(x) + self.Spatial_pos_embed_
        # x = rearrange(x, '(b f) p c -> b f (p c)', f=num_coeff_kept)
