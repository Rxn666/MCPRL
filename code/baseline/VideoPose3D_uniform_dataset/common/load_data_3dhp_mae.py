...
class Fusion(data.Dataset):
    def __init__(self, opt, root_path, train=True, MAE=False):
        ...
        if self.train:
            ...
            #                                                                     subset=self.subset)
            # print(self.valid_frame.keys())
            for ts in self.valid_frame.keys():
                self.valid_frame[ts] = np.ones((self.valid_frame[ts].shape))
                # print(self.valid_frame[ts].shape)
                # print(type(self.valid_frame[ts]))
    ...
    def prepare_data(self, path, train=True):
        ...
        if train == True:
            data = np.load(path+"data_train_3dhp_23.npz", allow_pickle=True)['data'].item()
