...
class opts():
    ...
    def init(self):
        ...
        self.parser.add_argument('-arc', '--architecture', default='3,3,3', type=str, metavar='LAYERS',
                            help='filter widths separated by comma')
        self.parser.add_argument('--causal', action='store_true', help='use causal convolutions for real-time processing')
        self.parser.add_argument('-ch', '--channels', default=1024, type=int, metavar='N',
                            help='number of channels in convolution layers')
        self.parser.add_argument('-drop', '--dropout', default=0.25, type=float, metavar='P', help='dropout probability')
